        ...
        function r(i) {
          if (n) {
            ...
            (s = { time: new Date().getTime(), coords: [t.pageX, t.pageY] }),
              Math.abs(n.coords[0] - s.coords[0]) >
                e.event.special.swipe.scrollSupressionThreshold &&
                i.preventDefault();
          }
        }
        i.bind("touchmove", r).one("touchend", function (t) {
          i.unbind("touchmove", r),
            n &&
              s &&
              s.time - n.time < e.event.special.swipe.durationThreshold &&
              Math.abs(n.coords[0] - s.coords[0]) >
                e.event.special.swipe.horizontalDistanceThreshold &&
              Math.abs(n.coords[1] - s.coords[1]) <
                e.event.special.swipe.verticalDistanceThreshold &&
              n.origin
                .trigger("swipe")
                .trigger(
                  n.coords[0] > s.coords[0] ? "swipeleft" : "swiperight"
                ),
            (n = s = void 0);
        });
      });
