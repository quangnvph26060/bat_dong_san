window.SLB &&
  ...
  (function ($) {
    var View = {
        ...
        options: {},
        _init: function () {
          this._super(), this.init_refs(), this.init_components();
        },
        init_refs: function () {
          var r, ref, prop;
          for (prop in this)
            if (
              ((prop = this[prop]),
              this.is_component(prop) &&
                !this.util.is_empty(prop.prototype._refs))
            )
              for (r in prop.prototype._refs)
                (ref = prop.prototype._refs[r]),
                  this.util.is_string(ref) &&
                    ref in this &&
            ...
            $.extend(!0, t.options, options),
              $(window).on("popstate", function (e) {
                ...
                if (t.util.in_obj(state, ["item", "viewer"]))
                  return (
                    t.get_viewer(state.viewer).history_handle(e),
                    e.preventDefault()
                  );
              }),
              t.init_items();
        ...
        get_component: function (type, id) {
          var ret = null;
          if (!this.util.is_func(type)) return ret;
          this.util.is_string(id) || (id = null);
          var coll = this.get_components(type);
          if (this.util.is_obj(coll)) {
            var tid = this.util.is_string(id)
              ? id
              : this.util.add_prefix("default");
            tid in coll && (ret = coll[tid]);
          }
          return (
            this.util.is_empty(ret) &&
              (this.util.is_string(id) ||
                this.can_make_default_component(type)) &&
              (ret = this.add_component(type, id)),
            ret
          );
        },
        add_component: function (type, id, options) {
          if (!this.util.is_func(type)) return !1;
          ...
          var m =
            "component" !== type.prototype._slug
              ? "add_" + type.prototype._slug
              : null;
          if (
            ((ret =
              !this.util.is_empty(m) && m in this && this.util.is_func(this[m])
                ? this[m](id, options)
                : new type(id, options)),
        ...
        add_component_temp: function (type) {
          var ret = null;
          return (
            this.is_component(type) &&
              ((ret = new type("")), (this.component_temps[ret._slug] = ret)),
            ret
          );
        },
        get_component_temp: function (type) {
          return this.has_component_temp(type)
            ? this.component_temps[type.prototype._slug]
            : this.add_component_temp(type);
        },
        has_component_temp: function (type) {
          return !!(
            this.is_component(type) &&
            type.prototype._slug in this.component_temps
          );
        },
        get_options: function (opts) {
          var ret = {};
          if (
            (this.util.is_string(opts) && (opts = [opts]),
        ...
        get_option: function (opt, def) {
          ...
          return (ret =
            this.util.is_obj(ret) && opt in ret
              ? ret[opt]
              : this.util.is_set(def)
              ? def
              : null);
        },
        ...
        get_viewer: function (v) {
          return (
            this.has_viewer(v) ||
              ...
              this.has_viewer(v) || (v = (v = this.add_viewer(v)).get_id())),
            this.get_viewers()[v]
          );
        },
        init_items: function () {
          var t = this,
            sel = this.util.format(
              'a[href][%s="%s"]',
              this.util.get_attribute("active"),
              1
            );
          $(document).on("click", sel, null, function () {
            var ret = t.show_item(this);
            return t.util.is_bool(ret) || (ret = !0), !ret;
          });
        },
        get_items: function () {
        ...
        get_item: function (ref) {
          if (this.util.is_type(ref, this.Content_Item)) return ref;
          var item = null;
          ...
          return (
            this.util.is_instance(item, this.Content_Item) ||
        ...
        save_item: function (item) {
          return this.util.is_instance(item, this.Content_Item)
        ...
        get_content_handler: function (item) {
          var type = this.util.is_instance(item, this.Content_Item)
        ...
        extend_content_handler: function (id, attr) {
          var hdl = null;
          if (!this.util.is_string(id) || !this.util.is_obj(attr)) return hdl;
          null === (hdl = this.get_content_handler(id))
            ? (this.get_content_handlers()[id] = hdl =
                new this.Content_Handler(id, attr))
        ...
        add_group: function (g, attrs) {
          return (
            (g = new this.Group(g, attrs)), (this.get_groups()[g.get_id()] = g)
          );
        },
        get_groups: function () {
        ...
        extend_theme: function (id, attr) {
          if (!this.util.is_string(id)) return !1;
          var dfr = $.Deferred();
          this.loading.push(dfr);
          var model = this.get_theme_model(id);
          return (
            this.util.is_empty(model) &&
              (model = this.save_theme_model({ parent: null, id: id })),
            this.util.is_obj(attr) &&
              ("id" in attr && delete attr.id, $.extend(model, attr)),
            this.util.in_obj(attr, "styles") && this.load_styles(attr.styles),
            this.util.is_obj(model.parent) ||
        ...
        save_theme_model: function (model) {
          return (
            this.util.in_obj(model, "id") &&
              this.util.is_string(model.id) &&
              (this.get_theme_models()[model.id] = model),
            model
          );
        },
        extend_template_tag_handler: function (id, attr) {
          if (!this.util.is_string(id) || !this.util.is_obj(attr)) return !1;
          var hdl,
            hdls = this.get_template_tag_handlers();
          return (
            this.util.in_obj(hdls, id)
              ? (hdl = hdls[id]).set_attributes(attr)
              : (hdls[
                  (hdl = new this.Template_Tag_Handler(id, attr)).get_id()
                ] = hdl),
            this.util.in_obj(attr, "styles") && this.load_styles(attr.styles),
        ...
        load_styles: function (styles) {
          if (this.util.is_array(styles)) {
            for (var style, out = [], x = 0; x < styles.length; x++)
              (style = styles[x]),
                this.util.in_obj(style, "uri") &&
                  this.util.is_string(style.uri) &&
                  out.push(
      ...
      Component = {
        ...
        get_id: function (ns) {
          var id = this._id;
          return this.util.is_bool(ns) && ns && (id = this.add_ns(id)), id;
        },
        get_ns: function () {
          return (
            null === this._ns && (this._ns = this.util.add_prefix(this._slug)),
            this._ns
          );
        },
        add_ns: function (val) {
        ...
        set_status: function (id, val) {
          return (
            this.util.is_string(id)
              ? (this.util.is_set(val) || (val = !0),
                this.util.is_obj(this._status, !1) || (this._status = {}),
                (this._status[id] = val))
              : this.util.is_set(val) || (val = !1),
            val
          );
        },
        ...
        get_component: function (cname, options) {
          var c = null;
          if (!this.has_reference(cname)) return c;
          options = $.extend({}, { check_attr: !0, get_default: !1 }, options);
          var ctype = this.get_reference(cname);
          return this.util.is_type(this[cname], ctype)
            ? this[cname]
            : ((c = this[cname] = null),
              options.check_attr &&
                ((c = this.get_attribute(cname)),
                this.util.is_empty(c) || (c = this.set_component(cname, c))),
              this.util.is_empty(c) &&
                options.get_default &&
                (c = this.get_controller().get_component(ctype)),
              c);
        },
        set_component: function (name, ref, validate) {
          if (!this.has_reference(name)) return null;
          if (this.util.is_empty(ref)) ref = null;
          else {
            var ctype = this.get_reference(name);
            this.util.is_string(ref, !1) &&
              (ref = this.get_controller().get_component(ctype, ref)),
              (this.util.is_type(ref, ctype) &&
                (!this.util.is_func(validate) || validate.call(this, ref))) ||
                (ref = null);
          }
          return (this[name] = ref), this[name];
        },
        ...
        init_attributes: function (force) {
          if (
            (this.util.is_bool(force) || (force = !1),
        ...
        init_default_attributes: function () {
          if (!this._attr_default_parsed && this.util.is_obj(this._attr_map)) {
            var opts = this.get_controller().get_options(
              this.util.obj_keys(this._attr_map)
            );
            if (this.util.is_obj(opts)) {
              for (var opt in this._attr_map)
                opt in opts &&
                  null !== this._attr_map[opt] &&
                  ((opts[this._attr_map[opt]] = opts[opt]), delete opts[opt]);
        ...
        get_dom_attributes: function () {
          var attrs = {},
            el = this.dom_get(null, { init: !1 });
          if (0 < el.length) {
            var attrs_full = $(el).get(0).attributes;
            if (this.util.is_obj(attrs_full)) {
              var attr_key,
                attr_prefix = this.util.get_attribute();
              $.each(attrs_full, function (idx, attr) {
                if (-1 === attr.name.indexOf(attr_prefix)) return !0;
        ...
        get_attributes: function () {
          return this.init_attributes(), this._attributes;
        },
        get_attribute: function (key, def, enforce_type) {
          if (
            (this.util.is_set(def) || (def = null), !this.util.is_string(key))
          )
            return def;
          ...
          return (
            enforce_type &&
              ret !== def &&
              null !== def &&
              !this.util.is_type(ret, $.type(def), !1) &&
              (this.util.is_scalar(def, !1) && this.util.is_scalar(ret, !1)
                ? this.util.is_string(def, !1)
                  ? (ret = ret.toString())
                  : this.util.is_num(def, !1) && !this.util.is_num(ret, !1)
                  ? ((ret = this.util.is_int(def, !1)
                      ? parseInt(ret)
                      : parseFloat(ret)),
                    this.util.is_num(ret, !1) || (ret = def))
                  : (ret = this.util.is_bool(def, !1)
                      ? this.util.is_string(ret) || this.util.is_num(ret)
                      : def)
                : (ret = def)),
            ret
          );
        ...
        dom_set: function (el) {
          return (
            (el = $(el)).data(this.get_data_key(), this),
            this._reciprocal && (this._dom = el),
            el
          );
        },
        dom_get: function (element, options) {
          var opts_default = { init: !0, put: !1 };
          (options = this.util.is_obj(options)
            ? $.extend({}, opts_default, options)
            : opts_default).init &&
            !this.get_status("dom_init") &&
            (this.set_status("dom_init"), this.dom_init());
          var ret = this._dom;
          if (ret && this.util.is_string(element)) {
            var ch = $(ret).find(this.dom_get_selector(element));
            ch.length
              ? (ret = ch)
              : (!0 !== options.put && !this.util.is_obj(options.put)) ||
        ...
        dom_put: function (element, content) {
          var r = null;
          if (!this.dom_has() || !this.util.is_string(element)) return $(r);
          var strip = ["tag", "content", "success"],
            options = { tag: "div", content: "", class: this.add_ns(element) };
          this.util.is_empty(content) ||
            (this.util.is_type(content, jQuery, !1) ||
            this.util.is_string(content, !1)
              ? (options.content = content)
              : this.util.is_obj(content, !1) && $.extend(options, content));
          for (var attrs = $.extend({}, options), x = 0; x < strip.length; x++)
            delete attrs[strip[x]];
          var d = this.dom_get();
          return (
            (r = $(this.dom_get_selector(element), d)).length ||
              ((r = $(this.util.format("<%s />", options.tag), attrs).appendTo(
                d
              )).length &&
                this.util.is_method(options, "success") &&
        ...
        on: function (event, fn, options) {
          ...
          this.util.is_obj(options, !1) || (options = {}),
            (options = $.extend({}, { clear: !1 }, options)),
            this.util.is_obj(this._events, !1) || (this._events = {});
          var es = this._events;
          return (
            (event in es &&
              this.util.is_obj(es[event], !1) &&
              !options.clear) ||
              (es[event] = []),
            es[event].push(fn),
            this
        ...
        trigger: function (event, data) {
          ...
          if (!(this.util.is_string(event) && event in this._events))
            return dfr.resolve(), dfr.promise();
          var ev = { type: event, data: null };
          return (
            this.util.is_set(data) && (ev.data = data),
    ...
    var Viewer = {
      _slug: "viewer",
      _refs: { item: "Content_Item", theme: "Theme" },
      _reciprocal: !0,
      _attr_default: {
        loop: !0,
        animate: !0,
      ...
      clear_item: function (full) {
        ...
        item && item.reset(), full && this.clear_component("item");
      },
      get_theme: function () {
        var ret = this.get_component("theme", { check_attr: !1 });
        return (
          this.util.is_empty(ret) &&
      ...
      get_lock: function (simple, full) {
        this.util.is_bool(simple) || (simple = !1),
          this.util.is_bool(full) || (full = !1);
        var s = "item_working";
        if (simple) return this.get_status(s);
        var r = this.get_status(s, !0);
        return (
          this.util.is_promise(r) || (r = this.lock()), full ? r : r.promise()
        );
      },
      is_locked: function () {
      ...
      show: function (item) {
        ...
        var vt = "theme_valid",
          valid = !0;
        if (
          (this.has_attribute(vt)
            ? (valid = this.get_attribute(vt, !0))
            : ((valid = !(
                !this.get_theme() ||
                "" === this.get_theme().get_template().get_layout(!1)
              )),
        ...
        function fin() {
          if (
            (v.lock(),
            v.set_status("show_deferred", !1),
            !v.set_item(v.item_queued))
          )
            return v.close(), !1;
          v.history_add(), v.set_active(), v.render();
        }
        var v = this;
        this.is_locked()
          ? this.get_status("show_deferred") ||
            (this.set_status("show_deferred"),
      ...
      history_handle: function (e) {
        ...
        if (this.util.is_string(state.item, !1))
          ...
          this.history_set(0), -1 !== count && this.close();
        }
      },
      ...
      history_add: function () {
        ...
        if (this.util.in_obj(opts, "event")) {
          ...
          this.util.in_obj(e, "state") &&
            this.util.in_obj(e.state, "count") &&
            (count = e.state.count);
        } else {
          var state = { viewer: this.get_id(), item: null, count: count };
      ...
      history_reset: function () {
        var count = this.history_get(!0);
        count && (this.history_set(-1), history.go(-1 * count));
      },
      is_open: function () {
      ...
      render: function () {
        var v = this,
          thm = this.get_theme();
        v.dom_prep(),
          this.get_status("render-events") ||
            (this.set_status("render-events"),
            thm
              .on("render-loading", function (ev, thm) {
                var dfr = $.Deferred();
                if (!v.is_active()) return dfr.reject(), dfr.promise();
                function set_pos() {
                ...
                return (
                  v.is_open()
                    ...
                    : thm
                        .transition("open")
                        .always(function () {
                          always(), v.events_open(), (v.open = !0);
                        })
                        .fail(function () {
                          set_pos(), v.get_overlay().show(), v.dom_get().show();
                        }),
                  dfr.promise()
                );
              })
              .on("render-complete", function (ev, thm) {
                if (!v.is_active()) return !1;
                var d = v.dom_get(),
      ...
      dom_init: function () {
        var d = this.dom_set(
            $("<div/>", { id: this.get_id(!0), class: this.get_ns() })
          )
            .appendTo(this.dom_get_container())
            .hide(),
          thm = this.get_theme();
        d.addClass(thm.get_classes(" "));
        var v = this;
        this.get_status("render-init") ||
      ...
      get_overlay: function () {
        var o = null,
          v = this;
        return (
          this.overlay_enabled() &&
            (o = this.dom_get("overlay", {
              put: {
                success: function () {
                  $(this)
                    .hide()
                    .css("opacity", v.get_attribute("overlay_opacity"));
                },
              },
            })),
          $(o)
      ...
      events_open: function () {
        if ((this.keys_enable(), this.open)) return !1;
      ...
      keys_control: function (ev) {
        ...
        if (
          ("rtl" === document.documentElement.getAttribute("dir") &&
            ((handlers[37] = this.item_next), (handlers[39] = this.item_prev)),
          ev.which in handlers)
        )
          return handlers[ev.which].call(this), !1;
      },
      slideshow_enabled: function () {
        var o = this.get_attribute("slideshow_enabled");
        return !(
          !(this.util.is_bool(o) && o && this.get_item()) ||
          this.get_item().get_group().is_single()
        );
      },
      slideshow_active: function () {
        return !(
          !this.slideshow_enabled() ||
          !(
            this.get_attribute("slideshow_active") ||
            (!this.init && this.get_attribute("slideshow_autostart"))
          )
        );
      },
      ...
      slideshow_set_timer: function (callback) {
        this.set_attribute(
          "slideshow_timer",
          setInterval(callback, 1e3 * this.get_attribute("slideshow_duration"))
        );
      },
      slideshow_start: function () {
        if (!this.slideshow_enabled()) return !1;
      ...
      slideshow_stop: function (full) {
        this.util.is_bool(full) || (full = !0),
          full &&
            (this.set_attribute("slideshow_active", !1),
      ...
      slideshow_pause: function (mode) {
        this.util.is_bool(mode) || (mode = !0),
          this.slideshow_active() &&
            (mode ? this.slideshow_stop(!1) : this.slideshow_start()),
    ...
    var Group = {
      ...
      get_selector: function () {
        return (
          this.util.is_empty(this.selector) &&
            (this.selector = this.util.format(
              'a[%s="%s"]',
              this.dom_get_attribute(),
              this.get_id()
            )),
          this.selector
        );
      ...
      get_item: function (idx) {
        ...
        var items = this.get_items(),
          max = this.get_size() - 1;
        return max < idx && (idx = max), items.get(idx);
      },
      get_pos: function (item) {
        return (
          this.util.is_empty(item) && (item = this.get_current()),
          this.util.is_type(item, View.Content_Item)
            ? this.get_items().index(item.dom_get())
            : -1
        );
      },
      ...
      get_current: function () {
        return (
          null === this.current ||
            this.util.is_type(this.current, View.Content_Item) ||
            (this.current = null),
          this.current
        );
      },
      ...
      get_next: function (item) {
        ...
        return (
          -1 !== pos &&
            ((0 === (pos = pos + 1 < this.get_size() ? pos + 1 : 0) &&
              !item.get_viewer().get_attribute("loop")) ||
      ...
      get_prev: function (item) {
        ...
        return (
          -1 === pos ||
            (0 === pos && !item.get_viewer().get_attribute("loop")) ||
            (0 === pos && (pos = this.get_size()),
            (pos -= 1),
      ...
      show_next: function (item) {
        if (1 < this.get_size()) {
          ...
          this.set_current(i), i.show(), this.trigger("item-next");
        }
      },
      show_prev: function (item) {
        if (1 < this.get_size()) {
          ...
          this.set_current(i), i.show(), this.trigger("item-prev");
        }
      },
      get_size: function () {
    ...
    var Content_Handler = {
      ...
      match: function (item) {
        var m = this.get_attribute("match");
        if (!this.util.is_empty(m)) {
          if (
            (this.util.is_string(m) &&
              ((m = new RegExp(m, "i")), this.set_attribute("match", m)),
            this.util.is_type(m, RegExp))
          )
            return m.test(item.get_uri());
          if (this.util.is_func(m)) return !!m.call(this, item);
        }
        return !1;
      },
      load: function (item) {
        var dfr = $.Deferred();
        return (
          null === this.call_attribute("load", item, dfr) && dfr.resolve(),
          dfr.promise()
        );
      },
      render: function (item) {
        var dfr = $.Deferred();
        return this.call_attribute("render", item, dfr), dfr.promise();
      },
    };
    ...
    var Content_Item = {
      _slug: "content_item",
      _reciprocal: !0,
      _refs: { viewer: "Viewer", group: "Group", type: "Content_Handler" },
      _attr_default: {
        source: null,
        permalink: null,
        dimensions: null,
      ...
      get_output: function () {
        ...
        if (this.util.is_string(ret)) dfr.resolve(ret);
        else if (this.has_type()) {
          var type = this.get_type(),
            item = this;
          type.render(this).done(function (output) {
            item.set_output(output), dfr.resolve(output);
          });
        } else dfr.resolve("");
        return dfr.promise();
      },
      ...
      get_uri: function (mode) {
        -1 === $.inArray(mode, ["source", "permalink"]) && (mode = "source");
        var ret = this.get_attribute(mode);
        return (
          this.util.is_string(ret) ||
            (ret = "source" === mode ? this.get_attribute("permalink") : ""),
          (ret = ret.replace(/&(#38|amp);/, "&"))
        );
      },
      get_title: function () {
        if (this.has_attribute("title_cached"))
          return this.get_attribute("title_cached", "");
        function validate(title) {
          return "string" != typeof title || "" === title.trim()
            ? ""
            : ((title = title.trim()),
              t.get_viewer().get_attribute("title_default") ||
                (title === t.get_title_default() && (title = "")),
              title);
        }
        var title = "",
          dom = this.dom_get(),
          ...
          (dom.length &&
            (title =
              (title =
                ...
                dom.closest("figure").find("figcaption").first().html()) ||
              dom.closest("figure").find(".wp-caption-text").first().html()),
          !title)
        )
          for (
            var props = ["caption", "title"], x = 0;
            x < props.length &&
            ((title = validate(this.get_attribute(props[x], ""))),
            this.util.is_empty(title));
            x++
          );
        return (
          !title &&
            ...
            (title =
              ...
              validate(dom.get(0).innerText.trim())),
          (title = validate(title)),
          this.set_attribute("title_cached", title),
          title
        );
      },
      get_title_default: function () {
        var val = "",
          prop = "title_default";
        if (this.has_attribute(prop)) val = this.get_attribute(prop);
        else {
          var f = this.get_uri("source"),
            i = f.lastIndexOf("/");
          -1 !== i &&
            -1 !== (i = (f = f.substr(i + 1)).lastIndexOf(".")) &&
            (f = f.substr(0, i)),
      ...
      gallery_type: function () {
        var ret = null,
          types = { wp: ".gallery-icon", ngg: ".ngg-gallery-thumbnail" },
          dom = this.dom_get();
        for (var type in types)
          if (0 < dom.parent(types[type]).length) {
            ret = type;
            break;
          }
        return ret;
      ...
      get_viewer: function () {
        return this.get_component("viewer", { get_default: !0 });
      },
      set_viewer: function (v) {
        return this.set_component("viewer", v);
      },
      get_group: function (set_current) {
        var g = this.get_component("group");
        return (
          g ||
            ((g = this.set_component("group", new View.Group())),
            (set_current = !0)),
      ...
      get_type: function () {
        var t = this.get_component("type", { check_attr: !1 });
        return (t =
      ...
      has_type: function () {
        return !this.util.is_empty(this.get_type());
      },
      show: function (options) {
        if (!this.has_type()) return !1;
        this.set_attribute("options_show", options);
        var v = this.get_viewer();
        return this.load(), v.show(this);
      },
      load: function () {
        return (
          this.util.is_promise(this.loaded) ||
    ...
    var Modeled_Component = {
      ...
      get_attribute: function (key, def, check_model, enforce_type) {
        ...
        return null === ret && (ret = this._super(key, def, enforce_type)), ret;
      },
      ...
      set_attribute: function (key, val, use_model) {
        if (!this.util.is_string(key) || !this.util.is_set(val)) return !1;
        (this.util.is_bool(use_model) ||
          this.util.is_obj(use_model) ||
          (use_model = !0),
        use_model)
          ? ((this.util.is_obj(use_model) ? use_model : this.get_model())[key] =
              val)
          : this._super(key, val);
        return val;
      },
      get_model: function () {
        ...
        return (
          this.util.is_obj(m) || ((m = {}), this.set_attribute("model", m, !1)),
          m
        );
      },
      has_model: function () {
      ...
      get_ancestors: function (inc_current) {
        for (var ret = [], m = this.get_model(); this.util.is_obj(m); )
          ret.push(m),
            (m =
              this.util.in_obj(m, "parent") && this.util.is_obj(m.parent)
                ? m.parent
                : null);
        return inc_current || ret.shift(), ret;
      },
      get_ancestor: function (attr, safe_mode) {
        if (!this.util.is_string(attr)) return !1;
        this.util.is_bool(safe_mode) || (safe_mode = !0);
        for (
          var mcurr = this.get_model(), m = mcurr, found = !1;
          this.util.is_obj(m);
        ...
        return (
          found ||
            (safe_mode
              ? (this.util.is_empty(m) && (m = mcurr),
                this.util.in_obj(m, attr) || (m[attr] = null))
              : (m = null)),
          m
        );
      },
    ...
    var Theme = {
      _slug: "theme",
      _refs: { viewer: "Viewer", template: "Template" },
      _models: {},
      _attr_default: { template: null, model: null },
      viewer: null,
      template: null,
      _c: function (id, attributes, viewer) {
        1 === arguments.length &&
          this.util.is_type(id, View.Viewer) &&
          ((viewer = id), (id = null)),
      ...
      get_model: function (id) {
        ...
        if (
          !this.util.is_set(id) &&
          this.util.is_obj(this.get_attribute("model", null, !1))
        )
          ret = this._super();
        else {
          ...
          this.util.is_string(id) ||
            ...
            this.util.in_obj(models, id) ||
              (id = $.map(models, function (v, key) {
                return key;
              })[0]),
            (ret = models[id]);
        }
      ...
      get_measurement: function (attr, def) {
        var meas = null;
        if (!this.util.is_string(attr)) return meas;
        ...
        var attr_cache = this.util.format("%s_cache", attr),
          ...
          w = $(window);
        (status in cache &&
          this.util.is_obj(cache._status) &&
          cache._status.width === w.width() &&
          cache._status.height === w.height()) ||
          (cache = {}),
          this.util.is_empty(cache) &&
            (cache._status = {
              width: w.width(),
              height: w.height(),
              index: [],
            });
        var pos = $.inArray(item, cache._status.index);
        return (
          -1 !== pos && pos in cache && (meas = cache[pos]),
          this.util.is_obj(meas) ||
            ((meas = this.call_attribute(attr)),
            this.util.is_obj(meas) ||
              (meas = this.get_measurement_default(attr))),
          (meas = this.util.is_obj(meas) ? $.extend({}, def, meas) : def),
          (pos = cache._status.index.push(item) - 1),
          (cache[pos] = meas),
      ...
      get_measurement_default: function (attr) {
        return this.util.is_string(attr)
          ? ((attr = this.util.format("get_%s_default", attr)),
            this.util.in_obj(this, attr)
              ? ((attr = this[attr]),
                this.util.is_func(attr) && (attr = attr.call(this)))
              : (attr = null),
            attr)
          : null;
      },
      ...
      get_offset_default: function () {
        ...
        if (l.length) {
          l.find("*").css({ width: "", height: "", display: "" });
      ...
      get_item_dimensions: function () {
        ...
        if (v.get_attribute("autofit", !1)) {
          ...
          (offset.height += margin.height), (offset.width += margin.width);
      ...
      render: function (init) {
        ...
        this.get_status(st) ||
          (this.set_status(st),
          tpl.on(
            ["render-init", "render-loading", "render-complete"],
            function (ev) {
        ...
        if (v.get_attribute(attr, !0) && this.util.is_string(event)) {
          clear_queue &&
            v
              ...
              .find("*")
              .each(function () {
                for (var el = $(this); el.queue().length; ) el.stop(!1, !0);
              });
          var trns,
            attr_set = [attr, "set"].join("_");
          if (this.get_attribute(attr_set)) trns = this.get_attribute(attr, {});
          else {
            var models = this.get_ancestors(!0);
            (trns = []), this.set_attribute(attr_set, !0);
            var thm = this;
            $.each(models, function (idx, model) {
              attr in model &&
                thm.util.is_obj(model[attr]) &&
                trns.push(model[attr]);
            }),
              trns.push({}),
              (trns = this.set_attribute(
                attr,
                $.extend.apply($, trns.reverse())
              ));
          }
          this.util.is_method(trns, event) &&
            (anim_on || ((fx_temp = $.fx.off), ($.fx.off = !0)),
            (dfr = trns[event].call(this, v, $.Deferred())));
        }
        return (
          this.util.is_promise(dfr) || (dfr = $.Deferred()).reject(),
          dfr.always(function () {
    ...
    var Template = {
      ...
      render: function (init) {
        ...
        if ((this.util.is_bool(init) || (init = !1), init))
          this.trigger("render-init", this.dom_get());
        else {
          if (!v.is_active()) return !1;
          var item = v.get_item();
          if (!this.util.is_type(item, View.Content_Item)) return v.close(), !1;
            ...
            $.when(item.load(), loading_promise).done(function () {
              return (
                ...
                ($.each(tags, function (idx, tag) {
                  if (!v.is_active()) return !1;
                  tag_promises.push(
                    tag.render(item).done(function (r) {
                      if (!v.is_active()) return !1;
                ...
                !!v.is_active() &&
                  void $.when.apply($, tag_promises).done(function () {
      ...
      get_layout: function (parsed) {
        return (
          ...
          parsed ? this.parse_layout() : this.get_attribute("layout_raw", "")
        );
      },
      parse_layout: function () {
        ...
        return (
          this.util.is_string(ret) ||
            ((ret = this.sanitize_layout(this.get_layout(!1))),
      ...
      sanitize_layout: function (l) {
        if (this.util.is_empty(l)) return l;
        ...
        switch (
          ...
          case "string":
            l = dom = dom.wrap("<div />").parent().html();
            break;
          default:
            l = dom;
        }
      ...
      parse_tags: function (l) {
        if (!this.util.is_string(l)) return "";
        for (
          var match, re = /\{{2}\s*(\w.*?)\s*\}{2}/gim;
          (match = re.exec(l));

        )
          l =
            l.substring(0, match.index) +
            this.get_tag_container(match[1]) +
            l.substring(match.index + match[0].length);
        return l;
      },
      ...
      get_tag: function (idx) {
        ...
        if (this.has_tags()) {
          ...
          (!this.util.is_int(idx) || idx < 0 || idx >= tags.length) &&
            (idx = 0),
            (ret = tags[idx]);
        }
        return ret;
      ...
      get_tags: function (name, prop, isolate) {
        ...
        if (!this.util.is_array(tags)) {
          ...
          $(nodes).each(function () {
            ...
            tag.has_handler() &&
              (tags.push(tag),
              isolate || (tag.dom_set(el), el.addClass(tag.get_classes(" ")))),
        ...
        if (!this.util.is_empty(tags) && this.util.is_string(name)) {
          ...
          for (var tags_filtered = [], tc = null, x = 0; x < tags.length; x++)
            name === (tc = tags[x]).get_name() &&
              ((prop && prop !== tc.get_prop()) || tags_filtered.push(tc));
          tags = tags_filtered;
        }
        return this.util.is_array(tags, !1) ? tags : [];
      },
      has_tags: function () {
        return 0 < this.get_tags().length;
      },
      ...
      dom_init: function () {
        this.dom_set(this.get_layout()), this.trigger("dom_init");
      },
      dom_get_tag: function (tag, prop) {
        var ret = $(),
    ...
    var Template_Tag = {
      _slug: "template_tag",
      _reciprocal: !0,
      _attr_default: { name: null, prop: null, match: null },
      handlers: {},
      _c: function (tag_match) {
        this.parse(tag_match);
      },
      parse: function (tag_match) {
        if (!this.util.is_string(tag_match)) return !1;
        var part,
          parts = tag_match.split("|");
        if (!parts.length) return null;
        var attrs = { name: null, prop: null, match: tag_match };
        (attrs.name = parts[0]),
          -1 !== attrs.name.indexOf(".") &&
            ((attrs.name = attrs.name.split(".", 2)),
            (attrs.prop = attrs.name[1]),
            (attrs.name = attrs.name[0]));
        for (var x = 1; x < parts.length; x++)
          1 < (part = parts[x].split(":", 1)).length &&
            !(part[0] in attrs) &&
            (attrs[part[0]] = part[1]);
      ...
      get_handler: function () {
        return this.has_handler()
          ? this.handlers[this.get_name()]
          : new View.Template_Tag_Handler("");
      },
      ...
      get_classes: function (rtype) {
        ...
        return this.util.is_string(rtype) && (cls = cls.join(rtype)), cls;
      },
      get_class: function (level) {
        ...
        switch (level) {
          ...
          case "full":
            var i,
              parts = [this.get_name(), this.get_prop()],
              a = [];
    ...
    var Template_Tag_Handler = {
      ...
      _attr_default: { supports_modifiers: !1, dynamic: !1, props: {} },
      render: function (item, instance) {
        var dfr = $.Deferred();
        return (
          this.call_attribute("render", item, instance, dfr), dfr.promise()
        );
      },
      add_prop: function (prop, fn) {
        var a = "props",
          props = this.get_attribute(a);
        if (!this.util.is_string(prop) || !this.util.is_func(fn)) return !1;
        this.util.is_obj(props, !1) || (props = {}),
          (props[prop] = fn),
          this.set_attribute(a, props);
      },
      handle_prop: function (prop, item, instance) {
        var props = this.get_attribute("props");
        return this.util.is_obj(props) &&
          prop in props &&
          this.util.is_func(props[prop])
          ? props[prop].call(this, item, instance)
          : item.get_viewer().get_label(prop);
      },
    };
